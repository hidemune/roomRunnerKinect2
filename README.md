# RoomRunner Kinect2 (treadmill Kinect2)

Linuxで動作する、Kinect2を用いたルームランナーです。
Kinextセンサーの前で走っている間のみ動画が再生されます。
動きが止まると動画は一時停止します。

# Setup
このソフトはlibfreenect2を改造して作成されました。

Ubuntuで作成しましたのでUbuntu用のセットアップ手順を以下に記述します。

* Setup.freenect2.shを実行する。
　　vlcや、その他依存するソフトがUbuntuにインストールされます。

* 以下のUbuntu用のセットアップを行う。

* Kinect2が正常に繋がった状態で、以下の実行ファイルでゲームスタート。
