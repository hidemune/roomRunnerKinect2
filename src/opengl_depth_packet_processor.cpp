...
public:
  ...
  void upload()
  {
    ...
    /*
    int y = 424/2;
    for (int x=50; x<512-50; x=x+1) {
      /*
      float f = 0.1;
      unsigned char *pc;
      pc = (unsigned char*)&f;
      // 0.6 in float
      pc[0] = data[(y*512+x)*4 + 0];
      pc[1] = data[(y*512+x)*4 + 1];
      pc[2] = data[(y*512+x)*4 + 2];
      pc[3] = data[(y*512+x)*4 + 3];
      * /
      std::cout << static_cast<unsigned short>(data[(y*512+x)] ) << std::endl;
    }
    std::cout << ">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>" << std::endl;
    // */
  }
...
void OpenGLDepthPacketProcessor::setConfiguration(const libfreenect2::DepthPacketProcessor::Config &config)
{
  ...
  /*
  //Hidemune
  std::cout << "Depth:min,max" << std::endl;
  std::cout << impl_->params.min_depth << std::endl;
  std::cout << impl_->params.max_depth << std::endl;
  */
...
void OpenGLDepthPacketProcessor::process(const DepthPacket &packet)
{
  ...
  std::cout << impl_->params.min_depth << std::endl;
  std::cout << impl_->params.max_depth << std::endl;
  std::cout << impl_->config.MinDepth << std::endl;
  std::cout << impl_->config.MaxDepth << std::endl;
  */
  ...
  for (int y=0; y<424; y++) {
    for (int x=0; x<512; x++) {
      ...
      // 0.6 in float
      if (true){
      pc[0] = depth->data[(y*512+x)*4 + 0];
      pc[1] = depth->data[(y*512+x)*4 + 1];
      pc[2] = depth->data[(y*512+x)*4 + 2];
      pc[3] = depth->data[(y*512+x)*4 + 3];
      }else{
      pc[0] = depth->data[(y*512+x)*4 + 3];
      pc[1] = depth->data[(y*512+x)*4 + 2];
      pc[2] = depth->data[(y*512+x)*4 + 1];
      pc[3] = depth->data[(y*512+x)*4 + 0];
      }
      double dist = f;
      //std::cout << ">>>Dist:" << static_cast<int>(dist) << std::endl;
...
system("xdotool search --name 'vlc with Kinect2' windowactivate &");
    ...
    (maxxL - minxL) > 3 || (maxyL - minyL) > 3) {
      ...
      std::cout << "stop......\t" << (maxx - minx) << "\t" << (maxy - miny) << "\t" << "\n";
