      ...
      else
      {
        ...
        else
        {
          ...
          for (int x=50; x<512-50; x=x+1) {
            ...
            unsigned char *pc;
            pc = (unsigned char*)&f;
            pc[0] = wb.data[(y*512+x)*4 + 0];
            pc[1] = wb.data[(y*512+x)*4 + 1];
            pc[2] = wb.data[(y*512+x)*4 + 2];
            pc[3] = wb.data[(y*512+x)*4 + 3];
            
            std::cout << ">>>" << std::endl;
            /* 4byte-gary-scale
            std::cout << static_cast<std::bitset<8> >(wb.data[(y*512+x)*4 + 0]) << std::endl;
            std::cout << static_cast<std::bitset<8> >(wb.data[(y*512+x)*4 + 1]) << std::endl;
            std::cout << static_cast<std::bitset<8> >(wb.data[(y*512+x)*4 + 2]) << std::endl;
            std::cout << static_cast<std::bitset<8> >(wb.data[(y*512+x)*4 + 3]) << std::endl;
             /
            std::cout << static_cast<int>(wb.data[(y*512+x)*4 + 0] & 0x1f) << std::endl;
